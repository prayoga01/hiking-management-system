...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Edit Data Mountain</h6>
        </div>
        <div class="card-body">
            <div class="container">
                <form method="POST" action="/mountainables/{{ $mountainAble->id }}">
                    @method('put')
                    @csrf
                    <div class="row mb-3">
                        {{-- <input type="hidden" name="id_mtn" value="{{ $mountainAble->id }}" /> --}}
                            ...
                            <select class="form-select" name="mountain_id" aria-label="Default select example">
                                @foreach ($mountains as $mountain)
                                    <option value="{{ $mountain->id }}" @if ($mountain->check_active == 0) disabled @endif
                                        @if ($mountainAble->mountain_id == $mountain->id) selected="selected" @endif>
                                        {{ $mountain->nm_mountain }} - {{ $mountain->address_mountain }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-8">
                            <input type="date" class="form-control @error('date_able') is-invalid @enderror"
                                id="date_able" name="date_able" value="{{ old('date_able', $mountainAble->date_able) }}">
                            @error('date_able')
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-8">
                            <input type="number" class="form-control @error('max_people') is-invalid @enderror"
                                ...
                                value="{{ old('max_people', $mountainAble->max_people) }}">
                            @error('max_people')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-8">
                            <input type="number" class="form-control @error('price') is-invalid @enderror" id="price"
                                name="price" value="{{ old('price', $mountainAble->price) }}">
                            @error('price')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
